...
PSAc_uc_h=rgamma(nsim,shape=96.04,scale=1.078151335)
PSAc_ls_p=rgamma(nsim,shape=96.04,scale=1.484859423)
PSAc_ls_h=rgamma(nsim,shape=96.04,scale=2.177489977)
PSAc_sex_p=rgamma(nsim,shape=96.04,scale=2.087856396)
PSAc_sex_h=rgamma(nsim,shape=96.04,scale=2.780486951)
# c_sex_s=3923.44 		
PSAc_rex_p=rgamma(nsim,shape=96.04,scale=4.212054115)
PSAc_rex_h=rgamma(nsim,shape=96.04,scale=4.90468467)
# c_rex_s=16819.53 		
PSAc_di_p=rgamma(nsim,shape=96.04,scale=5.49677489)
PSAc_di_h=rgamma(nsim,shape=96.04,scale=6.189405445)
# c_di_s=14119.86 		
...
#Utility
# u_ph=1	
PSAu_ph=rbeta(nsim,9.564430916,0.398517955)
PSAu_h=rbeta(nsim,9.564430916,0.398517955)
...
mean_zero_sbp=0
mean_zero_dbp=0
sd_zero_sbp=(0)/(2*1.96)
sd_zero_dbp=(0)/(2*1.96)
delta_zero_sbp=rnorm(nsim,mean = mean_zero_sbp, sd_zero_sbp)
delta_zero_dbp=rnorm(nsim,mean = mean_zero_dbp, sd_zero_dbp)
sbp_zero=sbp+delta_zero_sbp
dbp_zero=dbp+delta_zero_dbp
